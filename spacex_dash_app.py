...
def get_payload_scatter_plot(entered_site, payload_slider_value):
    filtered_df = spacex_df[['Launch Site','Payload Mass (kg)','class','Booster Version Category']]
    filtered_df = filtered_df[filtered_df['Payload Mass (kg)'].between(payload_slider_value[0],payload_slider_value[1])]
    if entered_site == 'ALL':
